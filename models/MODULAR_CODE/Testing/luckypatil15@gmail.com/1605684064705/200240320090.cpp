...
#include <string>
using namespace std;
class YourMOVIE_RATINg
{
    string mov1_zName; // stores movie name
    float totRating; // stores sum total of all ratings
    float totReviewer; // stores sum total of all reviewers

public:

    YourMOVIE_RATINg()  // zero parameter constructor
    {
        mov1_zName = """";
        totRating = -1;
        totReviewer = -1;
    }
    YourMOVIE_RATINg ( string movName, float totRating ) //parameterized constructor
    {
        this->mov1_zName = movName;
...
int search_Movie(string current_movie, vector <YourMOVIE_RATINg> movie_List) //search function to see if movie is already present
    {
...
int main()
{
    vector <YourMOVIE_RATINg> movie_List; //vector to store all movies
    ...
    while(1)
    {
        ...
        else
        {
            ...
            {
                YourMOVIE_RATINg newMovie(current_movie, current_rating);
