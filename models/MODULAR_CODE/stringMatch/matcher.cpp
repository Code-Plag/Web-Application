...
MATCHER::MATCHER(unsigned int Folder_Files_Index_1, unsigned int Folder_Files_Index_2, vector<TOKEN> file_1_token_vector, vector<TOKEN> file_2_token_vector, vector<MATCH> Tiles)

{
    ...
    this->Tiles = Tiles;
    this->Match_Percent_1 = 0;
    this->Match_Percent_2 = 0;
    if (file_1_token_vector.size() != 0 && file_2_token_vector.size() != 0)
    {
    ...
    for (unsigned int tiles_iterator = 0; tiles_iterator < Tiles.size(); tiles_iterator++)
    {
        ...
        {
            for (unsigned int line_iterator_file_1 = file_1_token_vector[Tiles[tiles_iterator].getIndex_Pattern() + match_iterator].getToken_Start_Line_no();
            ...
            for (unsigned int line_iterator_file_2 = file_2_token_vector[Tiles[tiles_iterator].getIndex_Text() + match_iterator].getToken_Start_Line_no();
...
MATCH MATCHER::getTiles_Match(unsigned int index)
{
    if (index < Tiles.size())
        return Tiles[index];
    else
...
unsigned int MATCHER::getFolder_Files_Index_1()
{
    return Folder_Files_Index_1;
}

unsigned int MATCHER::getFolder_Files_Index_2()
{
    return Folder_Files_Index_2;
}

unsigned int MATCHER::getTotal_Match_Token_Count()
{
...
unordered_map<string, unsigned int> MATCHER::getline_file1()
{
    return line_file1;
}
unordered_map<string, unsigned int> MATCHER::getline_file2()
{
    return line_file2;
}
